...
**但本项目不含 Concat 的任何代码**，剪辑层是用 FFmpeg 独立实现的，接口与数据结构都是自己设计的。Concat 采用 **AGPL-3.0**，若把它的源码并入，本项目就必须同样以 AGPL 开源；这里只借鉴理念、不合并代码，因此本项目采用 **Apache-2.0**。
...
- **分镜与生成结果闭环**：图片、视频保存到素材库后，会把 `image_asset_id` / `video_asset_id` 回写到来源分镜；图片生成多候选时可手动指定「分镜主图」。
- **创作资产分类**：素材库图片可标记为角色、场景、道具，并按用途筛选；不改变原有 `image` 资产类型，兼容旧数据。
- **@素材名引用**：图片与视频提示词支持 `@角色名`、`@场景名`、`@道具名` 自动补全和解析，生成时自动带入对应参考图；视频侧按角色参考、场景参考、普通参考区分用途，并保护首尾帧专用顺序。
- **分集生产与断点续作**：剧本、分镜、图片、视频、音频、任务中心和剪辑台按 `project_id / script_id / storyboard_id` 保持归属；项目重新打开时按真实集号恢复到最早未完成阶段，避免错集和错误跳转。
- **图片整集待办队列**：可一键提交当前集缺图分镜，逐镜使用各自 `image_prompt`，实时显示完成/失败进度；支持停止后续、仅重试失败项，以及离开/刷新后的暂停恢复，已绑定成功镜头不会重复生成。
- **视频 / 音频任务恢复**：视频异步任务持久化并按原任务上下文继续轮询；音频批量生成显示进度，支持停止后续与失败项重试，切换项目不会把旧任务写到新项目。
- **条漫编辑器增强**：支持常用画布预设、项目图片素材复用、元素复制，以及从项目分镜自动生成条漫初稿；已绑定的分镜图片优先进入画格，对白/旁白自动形成气泡或旁白块。
- **OpenReel Desktop 集成基础**：本地服务可发现并启动 OpenReel Desktop、读取本地 MCP 端点、创建项目/导入媒体/映射时间线；Web OpenReel 与简易剪辑台继续作为回退方案。
- **剪辑稳定性**：加强裁剪、速度、音量、转场、布尔参数校验；修复媒体路径穿越与 Range 解析；完善本地大文件暂存、任务恢复与 OpenReel 时间线映射。
- **工程质量**：API 冒烟测试改为 Node；补充剪辑、FFmpeg 渲染、渲染精度、HTTP/API 回归测试；修复字体导入顺序、对象 URL 清理、写锁残留等问题。

> 当前图片生产已支持自动连续消费整集待办队列；视频仍以异步任务提交/恢复与分镜待办队列为主，尚未做成与图片完全相同的一键整集连续提交体验。条漫长图真实渲染导出也仍在完善。README 不把未完成能力写成已完成。
...
| 图片生成 | 文生图 / 图生图 / 多图参考；支持分镜结果回写、分镜主图、`@素材名` 参考资产，以及整集缺图队列、进度、停止后续、失败重试和断点恢复 |
| 视频生成 | 异步任务提交与状态轮询；支持分镜结果回写、`@素材名` 参考资产、任务刷新恢复与失败重试 |
| 音频制作 | 声音库 + TTS，支持自定义 TTS 接口与分镜对白/旁白绑定；批量配音支持进度、停止后续和失败项重试 |
| **剪辑台** | 简易 FFmpeg 剪辑 + OpenReel Desktop / Web 集成与回退 |
| 素材库 | 图片 / 视频 / 音频 / 文本统一管理；图片可分类为角色 / 场景 / 道具 |
| 条漫编辑器 | 项目图片复用、画布预设、元素编辑、按分镜自动组装条漫初稿 |
...
├── shared/              前端与服务端共用的规则（模型能力判断）
├── editor-dist/         剪辑台（OpenReel）构建产物，不入库，见下
└── docs/                历史需求与设计文档（部分内容仍描述已移除的登录/管理员，仅供参考）
```

### 剪辑台

剪辑台目前采用三级策略：优先连接 **OpenReel Desktop** 的本地 MCP；Desktop 不可用时可回退到托管在 `/editor/` 的 OpenReel Web；同时保留本项目自带的简易 FFmpeg 剪辑台。

| | OpenReel Desktop | OpenReel Web | 简易剪辑台 |
|---|---|---|---|
| 实现 | [OpenReel Video](https://github.com/Augani/openreel-video) Desktop（MIT） | OpenReel 独立 Web 构建 | 本项目自带 |
| 接入 | 本地 MCP（仅 `127.0.0.1` + bearer token） | `/editor/` iframe 隔离 | 本地 HTTP |
| 能力 | 多轨工程、媒体导入、时间线映射 | 多轨、关键帧、调色、音频效果 | 分割 / 裁剪 / 变速 / 静音 / 反转 / 淡入，导出 H.264 |
| 素材来源 | 本项目可暂存并导入本地素材 | 手动导入 / 素材链接 | **与本地素材库打通** |
| 渲染 | OpenReel Desktop | 浏览器 WebCodecs / WebGPU | 本机 FFmpeg |

OpenReel 是**独立构建的应用**（React 19），与本项目（React 18）不能同构建——
同一棵依赖树里放不下两个 React。所以它由本地服务托管在 `/editor/`，
用 iframe 隔离加载：两套 React 各跑各的，互不干扰。

它的构建产物（约 59 MB）会内联进 exe，所以带完整剪辑台的 exe 约 169 MB
（不带的话是 84 MB）。产物目录 `editor-dist/` 不入库，需要时按下面重建。

**重建剪辑台：**

```bash
# OpenReel 是 pnpm workspace（npm 不认 workspace: 协议）
cd <openreel 源码>
npx pnpm@9 install
cd apps/web
npx vite build --base=/editor/      # 必须带 base，否则资源路径会指向站点根
# 把 apps/web/dist/* 复制到本项目的 editor-dist/
```

> ⚠️ 不要用 `OPENREEL_DESKTOP=1` 构建：那个模式会用 `strip-ffmpeg` 插件
> 把 FFmpeg 的 wasm 换成空字符串，导出功能就废了。
