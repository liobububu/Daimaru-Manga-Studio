...
- **分镜批处理入口**：分镜卡片支持多选、全选、批量生图、批量出片，并把所选分镜队列传给下游，为连续生产队列打基础。
...
> 当前批量生图/批量出片已经建立分镜队列入口，但「自动连续消费整个队列」仍是后续工作；条漫长图真实渲染导出也仍在完善。README 不把未完成能力写成已完成。
...
| 图片生成 | 文生图 / 图生图 / 多图参考；支持分镜结果回写、分镜主图、`@素材名` 参考资产 |
| 视频生成 | 异步任务提交与状态轮询；支持分镜结果回写与 `@素材名` 参考资产 |
| 音频制作 | 声音库 + TTS，支持自定义 TTS 接口与分镜对白/旁白绑定 |
